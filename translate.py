...
############# To be removed once API is implemented
import inquirer
...
def download_file(service, folder_id, temp_dir):
  query = f"'{folder_id}' in parents" # and mimeType contains 'audio/'" # change 'audio/' using api call
...
def main():
  ...
  # Download audio file
  input_audio = download_file(service, input_folder, temp_dir.name)
  ...
  questions = [
      inquirer.List('choice',
                    message="Select an Initial Language",
                    choices=['English', 'Spanish', 'French', 'Italian', 'German', 'Arabic', 'Chinese', 'Hindi', 'Russian', 'Japanese'],
                    ),
  ]

  input1 = inquirer.prompt(questions)
  init_lang = nlp_codes.get(input1['choice'])

  ######################### INPUT TO CHANGE: Language to translate to
  questions = [
      inquirer.List('choice',
                    message="Select an Output Language",
                    choices=['English', 'Spanish', 'French', 'Italian', 'German', 'Arabic', 'Chinese', 'Hindi', 'Russian', 'Japanese'],
                    ),
  ]

  input2 = inquirer.prompt(questions)
